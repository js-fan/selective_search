...
OPENCV_INCLUDE = -I/usr/local/opencv/include
OPENCV_LIBRARY = -L/usr/local/opencv/lib
...
wraper: $(OBJFILES) python/wraper.cpp
	$(CXX) -o python/selective_search/selective_search.so $^ $(CFLAGS) $(CLINKS) -shared
...
clean:
	rm -f build/*.o build/*.so python/selective_search/selective_search.so example
